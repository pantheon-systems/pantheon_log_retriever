...
if ((not args.uuid) or (not args.env)):
  ...
  for ip in app_ips:
    app_process = subprocess.Popen("rsync -rlvz --size-only --ipv4 --progress -e 'ssh -p 2222' " + args.env + "." + args.uuid + "@appserver." + args.env + "." + args.uuid + ".drush.in:logs/* app_server_logs_" + ip, shell=True)
    app_process.wait()
  ...
  app_ips = app_ips.split('\n')
  for ip in app_ips:
    app_process = subprocess.Popen("rsync -rlvz --size-only --ipv4 --progress -e 'ssh -p 2222' " + args.env + "." + args.uuid + "@dbserver." + args.env + "." + args.uuid + ".drush.in:logs/* db_server_logs_" + ip, shell=True)
    app_process.wait()
else:
